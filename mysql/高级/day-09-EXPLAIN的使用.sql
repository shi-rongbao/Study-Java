...
# 子查询，有两个SELECT，因此EXPLAIN时id最大是2
SELECT * FROM s1 
...
# 有两个SELECT，因此EXPLAIN时id最大是2
EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key1 FROM s2) OR key3 = 'a';
 
###### 查询优化器可能对涉及子查询的查询语句进行重写,转变为多表查询的操作########
# 子查询相当于循环的嵌套，时间复杂度为O(N ^ n), 如果能被优化，优化器会帮我们自动优化成多表查询，时间复杂度O(N)
EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key2 FROM s2 WHERE common_field = 'a');

# UNION去重会出现临时表，因此这是EXPLAIN时会出现三条数据
EXPLAIN SELECT * FROM s1 UNION SELECT * FROM s2;

# UNION ALL不去重，因此不会有临时表的产生，因此只有两条数据
EXPLAIN SELECT * FROM s1  UNION ALL SELECT * FROM s2;

/**
	id如果相同，可以认为是一组，从上往下顺序执行
	在所以组中，id值越大，优先级越高，越先执行
	关注点：id号每个号码，表示一趟独立的查询，一个sql的查询趟输越少越好
*/
...
EXPLAIN SELECT * FROM s1 UNION SELECT * FROM s2;  # 三张表，存在临时表

EXPLAIN SELECT * FROM s1 UNION ALL SELECT * FROM s2;  # 两张表，两条数据

# 子查询：
# 如果包含子查询的查询语句不能够转为对应的`semi-join`的形式，并且该子查询是不相关子查询。
# 该子查询的第一个`SELECT`关键字代表的那个查询的`select_type`就是`SUBQUERY`  # id越大越先执行，因此子查询中的先执行
EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key1 FROM s2) OR key3 = 'a';
...
# 那么对该表的访问方法就是`system`。 该记录的性能最高
# MYISAM存储引擎中会有一个变量记录COUNT(*)的值
...
# `const`是常量级别的，除了system以外性能最好的
EXPLAIN SELECT * FROM s1 WHERE id = 10005;

EXPLAIN SELECT * FROM s1 WHERE key2 = 10066;

# key3是普通索引, 不是聚簇索引，也不是唯一的二级索引，type = ref
# 使用索引要保证最左前置原则
EXPLAIN SELECT * FROM s1 WHERE key3 = '312';

/**
	练习：
		VARCHAR(10) 变长字段且允许NULL = 10 * （character set: utf=3, gbk=2, latinl=1）+ 1(NULL) + 2 (变长字段)
		VARCHAR(10) 变长字段且不允许NULL = 10 * (character set: utf=3, gbk=2, latinl=1）+ 2 (变长字段)
		CHAR(10) 固定字段且允许NULL = 10 * （character set: utf=3, gbk=2, latinl=1）+ 1(NULL)
		CHAR(10) 固定字段且不允许NULL = 10 * （character set: utf=3, gbk=2, latinl=1）
*/
...
EXPLAIN SELECT * FROM s1 WHERE 1 != 1;  # Impossible WHERE  where 条件永远是false


# 当我们使用全表扫描来执行对某个表的查询，并且该语句的`WHERE`子句中有针对该表的搜索条件时，在`Extra`列中会提示上述额外信息。
EXPLAIN SELECT * FROM s1 WHERE common_field = 'a';  # 没有使用索引
...
EXPLAIN SELECT key1, id FROM s1 WHERE key1 = 'a';  # 覆盖索引
...
-> Nested loop inner join  (cost=1395 rows=1015)
...
FROM sys.`innodb_buffer_stats_by_table` ORDER BY allocated
LIMIT 10;