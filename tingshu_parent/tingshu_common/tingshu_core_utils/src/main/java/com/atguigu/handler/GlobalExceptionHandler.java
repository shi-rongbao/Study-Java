...
public class GlobalExceptionHandler {
    ...
    @ResponseBody
    public RetVal<List<String>> methodArgumentNotValidException(MethodArgumentNotValidException e) {
        BindingResult bindingResult = e.getBindingResult();
        List<ObjectError> errorList = bindingResult.getAllErrors();
        List<String> errorMsgList = errorList.stream().map(error -> {
            log.error("字段：{}, 信息：{}", error.getObjectName(), error.getDefaultMessage());
