...
public class RateLimitController {
    ...
    // value的值就是sentinel中簇点链路的值，blockHandler的只就是服务限流后的结果的方法名
    public String byResource() {
        return "按照资源名称SentinelResource限流测试";
    }

    public String handlerBlockHandler(BlockException blockException) {
        return "服务不可用触发@SentinelResouce";
    }

    @GetMapping("/rateLimit/doAction/{p1}")
    @SentinelResource(value = "doActionSentinelResource",
            blockHandler = "doActionBlockHandler", fallback = "doActionFallback")  // 异常执行fallback，限流执行blockHandler
